...
#
# Usage:
#   ./scripts/github.sh status                # check auth
#   ./scripts/github.sh sync                  # fetch + fast-forward main
#   ./scripts/github.sh list                  # list ready issues
#   ./scripts/github.sh view <number>         # view issue
#   ./scripts/github.sh start <number>        # claim + branch + label
#   ./scripts/github.sh create "<title>"      # create issue with template body
#   ./scripts/github.sh create --title "<t>" --body-file /tmp/body.md --label "chore" --repo "owner/repo"
#   ./scripts/github.sh pr <number>           # push + create PR (auto title/body)
#   ./scripts/github.sh pr <number> --title "<t>" --body-file /tmp/body.md --repo "owner/repo"
#   ./scripts/github.sh push                      # push current branch (for updating PRs)
...
TEMPLATE
  ...
  help|*)
    echo "Usage: github.sh {status|sync|list|view|start|create|pr|push|update|finish|done} [args]"
    echo ""
    echo "Commands:"
    echo "  status              Check gh auth"
    echo "  sync                Fetch + fast-forward main from origin"
